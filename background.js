    ...
    try {
      switch (msg?.type) {
        ...
        case "save-sheets-config":
          ...
          return;
        case "push-to-sheets":
          sendResponse(await pushToSheets(msg.scope));
          return;
...
async function resumeProcessing() {
  ...
  notifyStatusUpdated();
  toast(
    'Listo para reanudar. Usa "Procesar lista" o "Pausar/Continuar".',
    "ok"
  );
}
...
function buildCustomDescription(min, max, genders) {
  const mapped = genders.map((g) =>
    g === "M" ? "Hombres" : g === "F" ? "Mujeres" : g
  );
  ...
  if (cleanUrl) {
    ...
    try {
      ...
      if (e instanceof TypeError) {
        throw new Error(
          "Invalid webhook URL format. Please provide a valid URL."
        );
      }
      throw e;
    }
    if (parsed.protocol !== "https:")
      throw new Error("Use HTTPS for the webhook.");
  }
  await chrome.storage.local.set({
...
async function pushToSheets(scope = "current") {
  ...
  // Construir URL con apiKey si se configuró en el popup
  let url = cfg.url;
  if (cfg.apiKey) {
  ...
  try {
    res = await fetch(url, {
      method: "POST",
      headers: {
        // SOLO Content-Type, para evitar preflight CORS innecesario
    ...
    clearTimeout(timer);
    console.error("Error en fetch a Sheets:", e);
    throw e;
  }
  clearTimeout(timer);

  if (!res.ok) {
    throw new Error(`Sheets respondió ${res.status}`);
  }
...
async function buildSheetsPayload(scope = "current") {
  ...
  if (snap && isRowValid(snap))
    payloadRows.push({ Rol: st.currentRole, ...snap });
...
function getMergedGenderDictObject() {
  ...
  return (async () => {
    ...
    for (const [k, v] of Object.entries(usr))
      merged[k.toLowerCase()] = (v || "").toUpperCase();
    // overrides
    for (const [k, v] of Object.entries(ovs))
      merged[k.toLowerCase()] = (v || "").toUpperCase();
    return merged;
  })();
}
...
async function importGenderDict({ text, mode }) {
  ...
  toast(
    `Diccionario importado (${Object.keys(parsed).length} entradas, modo: ${
      mode || "merge"
    }).`,
    "ok"
  );
...
async function scanAllTabsAndQueue() {
  ...
  if (totalAdded)
    toast(`Auto-scan: +${totalAdded} enlaces agregados a la cola.`, "ok");
}
...
async function startProcessing() {
  ...
  if (st.holdProcessing) {
    toast(
      'Está detenido. Usa "Reanudar" para permitir procesar de nuevo.',
      "warn"
    );
    return;
  }
...
async function processLoop() {
  while (true) {
    ...
    try {
      ...
      const completed = await ensureGender(row, tabId);
      const decided = await decideAndOptionallyClick(
        tabId,
        completed,
        st.estadoMode
      );
...
function promptGenderOnPage(tabId, firstName, guess) {
  return new Promise((resolve) => {
    const id =
      "gender_" + Date.now() + "_" + Math.random().toString(36).slice(2);
      ...
      const ok = await safeExecuteScript(
        ...
        (t) => {
          ...
          const btn = all.find((b) => {
            ...
            return (
              /mover a/i.test(txt) &&
              (isSel ? /seleccionad/.test(txt) : /descartad/.test(txt))
            );
          });
          if (btn) {
            btn.click();
...
function decideByRules(role, row, customRules) {
  ...
  if (custom) {
    ...
    if (
      ...
      age >= custom.ageMin &&
      age <= custom.ageMax &&
      custom.genders.includes(gender)
    ) {
...
async function saveRow(row) {
  ...
  let exists = false;
  if (doc)
    exists = rows.some((r) => (r.Documento || "").toString().trim() === doc);
  else if (email)
    exists = rows.some((r) => (r.Email || "").toLowerCase().trim() === email);
    ...
    try {
      ...
      sheetsPushTimer = setTimeout(() => {
        pushToSheets("current").catch((e) => {
          ...
          toast(
            "Error al enviar automático a Sheets: " + (e?.message || e),
            "err"
          );
        });
      }, 5000);
    } catch (e) {
      console.error("Error programando auto-push a Sheets:", e);
    }
  }
}
...
async function exportAllXls() {
  ...
  const xml = buildExcelXml(sheetName, headersAll, merged, {
    autoFilter: true,
  });
...
function normalizeFirstName(fullName) {
  ...
  const tokens = s
    .split(/\s+/)
    .filter((t) => !["de", "del", "la", "las", "los", "y", "e"].includes(t));
  return tokens[0] || "";
}
...
function buildExcelXml(sheetName, headers, rows, opts = {}) {
  ...
  const colsXml = headers
    .map(
      (h) =>
        `<Column ss:AutoFitWidth="1" ss:Width="${charToPts(
          maxLen[h] + 2
        ).toFixed(1)}"/>`
    )
    .join("");
  ...
  const headerRow =
    ...
    headers
      .map(
        (h) =>
          `<Cell ss:StyleID="sHeader"><Data ss:Type="String">${escXml(
            h
          )}</Data></Cell>`
      )
      .join("") +
  ...
  const dataRows = rows
    .map(
      (r) =>
        ...
        headers
          .map(
            (h) => `<Cell><Data ss:Type="String">${escXml(r[h])}</Data></Cell>`
          )
          .join("") +
        `</Row>`
    )
  ...
  const autoFilter = opts.autoFilter
    ? `
    <AutoFilter x:Range="R1C1:R${rows.length + 1}C${
        headers.length
...
function extractOnPage() {
  ...
  if (panel) {
    ...
    const liEmail = lis.find(
      (li) =>
        ...
        /e-?mail/i.test(
          li.querySelector(".icon")?.getAttribute("title") || ""
        ) ||
    ...
    const liDoc = lis.find(
      (li) =>
        /identific|cedula|c[ée]dula|dni|document/i.test(
          li.querySelector(".icon")?.getAttribute("title") || ""
  ...
  const digits = Array.from(
    new Set(phones.map((s) => s.replace(/\D/g, "")).filter(Boolean))
  );
  ...
  const others = normalized.filter(
    (d) =>
      !(d.length === 10 && d.startsWith("3")) && d.length >= 7 && d.length <= 10
  );
...
function clickMoveButtonOnPage(target) {
  ...
  const btn = all.find((b) => {
    ...
    return (
      /mover a/i.test(t) &&
      (isSel ? /seleccionad/.test(t) : /descartad/.test(t))
    );
  });
  if (btn) {
    btn.click();
