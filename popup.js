...
const btnExportAllXls = document.getElementById("btnExportAllXls");
...
async function renameRole() {
  ...
  if (!newName) {
    alert("Ingresa el nuevo nombre del rol.");
    return;
  }
  try {
...
async function refreshStatus() {
  ...
  if (st.domWarning && st.domWarning.count) {
    ...
    domWarningInfo.textContent =
      `AVISO: Posible cambio en Computrabajo (${w.count} fallos de extracción). ` +
      `Última URL: ${w.lastUrl || "N/D"}`;
    domWarningInfo.style.color = "#e8a23a";
  } else {
    domWarningInfo.textContent = "";
  }
